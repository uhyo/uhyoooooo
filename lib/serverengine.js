...
//サーバー用ファイル読み込み
Game.prototype.readFile=function(filename,option,callback){
	...
	var fs=require('fs');
	var re=option.responseType || "text";
	var enc= re==="buffer" ? null : "utf8";
	fs.readFile(filename,enc,function(err,data){
		...
		if(re==="json"){
			callback(err ? null : JSON.parse(data));
		}else{
...
ServerTransporter.prototype={
	...
	add:function(obj){
		var o={
			constructorName:obj._constructor.name,
			_id:obj._id,
			param:this.game.jsonFilter(obj._param,2),
		};
		if(obj._private){
	...
	event:function(obj,name,args){
		...
		this.broadcast("event",{
			_id:obj._id,
			name:name,
			args:this.game.jsonFilter(args,0),
		});
	},
