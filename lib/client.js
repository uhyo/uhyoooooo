...
gaminginfo.on("new",function(game){
	socket.on("init",function(obj){
		...
		socket.on("die",function(_id){
			...
			game.objectsmap[_id].event.emit("die");	//dieシグナル
		...
		socket.on("event",function(obj){
			...
			if(!o)return;
			o.event._old_emit.apply(o.event,[obj.name].concat(game.executeJSON(obj.args)));
		});
...
Game.prototype.initObject=function(d){
	//クライアント側でのemitを抑制
	d.event._old_emit=d.event.emit;
	d.event.emit=function(){};
	//イベントを制限 die無効
