/*
   uhyoooooo - node.js socket.io game module

   Uhyo (Ryota Suzuki)
   MIT licensed.

   http://github.com/uhyo/uhyoooooo

   */
if(typeof require==="function" && typeof EventEmitter==="undefined"){
	EventEmitter=require('events').EventEmitter;
}else if("undefined"===typeof EventEmitter){
	//include client EventEmitter(http://github.com/Wolfy87/EventEmitter)
...
Game.prototype.executeJSON=function(obj){
	...
	if(obj.$type==="user"){
		...
		if(obj._id==this.user._id){
			...
			if(this.objectsmap[obj._id]){
				//前に作ったのがある
				user=this.objectsmap[obj._id];
			}else{
				user=this.newUser();
				user.internal=false;
				user._id=obj._id;
...
Game.ClientCanvasView.prototype=Game.util.merge(new Game.ClientView,{
	init:function(callback){
		//パラメータはcallbackを読んで得る
		document.addEventListener("DOMContentLoaded",function handler(e){
			var param= ("function"===typeof callback) ? callback() : (("object"===typeof callback) && callback ? callback : {});
...
Game.ClientDOMView.prototype=Game.util.extend(Game.ClientView,{
	init:function(callback){
		...
		document.addEventListener("DOMContentLoaded",function handler(e){
			var param= ("function"===typeof callback) ? callback() : (("object"===typeof callback) && callback ? callback : {});
			//param.root: 親になるやつ（その下に描画する。デフォルト:document.body
			t.root= param.root || document.body;
			Game.ClientView.prototype.init.apply(t,arguments);
			t.nodeMap={};	//_idをキーにしたい
			/*(_id):{
			  node:(Node)
			  dependency:[obj,obj,...]
			  }*/
	...
	render:function(obj){
		...
		if(mm.rendering)return mm.node;	//already rendering now!
		if(this.stacktop){
			//そのオブジェクトに依存する
